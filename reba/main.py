...
class Landmark(BaseModel):
    ...
    # x, y は必須とするバリデータ (もしデータに含まれないケースがあればOptionalにする)
    @validator('x', 'y', pre=True, always=True)
    def check_presence(cls, v, field):
        if v is None:
            raise ValueError(f"Field '{field.name}' must be present and non-null")
        # Ensure it's a valid float
        try:
            float(v)
        except (ValueError, TypeError):
             raise ValueError(f"Field '{field.name}' must be a valid number")
        return v
...
class CalibrationInputs(BaseModel):
    ...
    shockForce: int = Field(..., description="Shock/Rapid force flag (0 or 1)") # 衝撃力フラグ
    ...
    def supporting_leg_valid(cls, v, values): # Include values to access other fields
        # Allow None only if not standingOne
        posture = values.get('postureCategory')
        if posture == 'standingOne' and v not in ['left', 'right']:
    ...
    # Add validators for other flags (0/1, 0/-1) if needed
...
def calc_forearm_score(elbowAngle: Optional[float]) -> int:
    ...
    return 1 if 80 <= internal_angle <= 120 else 2 # Corrected range based on internal angle
...
def getScoreA(trunk: int, neck: int, leg: int, loadKgInput: float, shockForceFlag: int) -> int: # Added shockForceFlag
    # Max values based on revised scoring functions
...
def get_final_reba_score(landmarks: List[Landmark], calib: CalibrationInputs) -> Dict[str, Any]:
    ...
    try:
        ...
        # --- Leg Score ---
        left_knee_angle = angles.get("leftKneeAngle")
        ...
        upperArmScore = calc_upper_arm_score_revised(
            angles.get(f"{side_to_eval_arm}UpperArmAngleMagnitude", 0), angles.get(f"{side_to_eval_arm}UpperArmIsExtension", False),
...
async def compute_reba_endpoint(input_data: REBAInput):
    try:
        # Add basic check for landmark count before detailed validation if needed
        if not input_data.landmarks or len(input_data.landmarks) < 33: # Check for sufficient landmarks early
             raise HTTPException(status_code=400, detail=f"Insufficient landmarks provided ({len(input_data.landmarks)}). Need 33.")

        # Pydantic validation runs implicitly here if using newer FastAPI/Pydantic
        # For older versions or explicit check:
        # try:
        #     validated_input = REBAInput.model_validate(input_data.model_dump()) # Pydantic v2
        #     # validated_input = REBAInput(**input_data.dict()) # Pydantic v1
        # except ValidationError as e:
        #     raise HTTPException(status_code=422, detail=e.errors())
        # result = get_final_reba_score(validated_input.landmarks, validated_input.calibInputs)
